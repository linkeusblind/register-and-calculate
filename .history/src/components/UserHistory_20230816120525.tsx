                ...
                <Table>
                    ...
                    <TableBody>
                        {userHistory.map((historyItem, index) => (
                            <TableRow key={index}>
                                {[
                                    <TableCell key="userName">{historyItem.userName}</TableCell>,
                                    <TableCell key="numbers">
                                        {`${historyItem.num1} ${historyItem.operator} ${historyItem.num2}`}
                                    </TableCell>,
                                    <TableCell key="operator">{historyItem.operator}</TableCell>,
                                    <TableCell key="result">{historyItem.result}</TableCell>,
                                    <TableCell key="timestamp">{historyItem.timestamp}</TableCell>,
                                ]}
                            </TableRow>
                        ))}
                    </TableBody>
